...
class Household:
  ...
  def AteOutUpdate(self):
    self._ChanceEatOutPerDay = random.random()

  def NotEatOutUpdate(self):
    self._ChanceEatOutPerDay = min(1.0, self._ChanceEatOutPerDay + 0.1)
...
class Settlement:
  ...
  def SubHousehold(self):
    if self._Households:
        self._Households.pop()

  def KillBuggyHouseholds(self):
    for House in self._Households:
        X = House.GetX()
        Y = House.GetY()
        if not (0 <= X < self._XSize and 0 <= Y < self._YSize):
            NewX = random.randint(0, self._XSize - 1)
            NewY = random.randint(0, self._YSize - 1)
            House._XCoord = NewX
            House._YCoord = NewY
...
class SmallSettlement(Settlement):
    def __init__(self, SmallXSize, SmallYSize, FewerHouseholds):
        super(SmallSettlement, self).__init__()
        self._XSize -= SmallXSize
        self._YSize -= SmallYSize
        self._StartNoOfHouseholds -= FewerHouseholds
        for Count in range(1, FewerHouseholds + 1):
            self.SubHousehold()
        self.KillBuggyHouseholds()
...
class Outlet:
  ...
  def GetDetails(self):
    ...
    Details += str(self._MaxCapacity) + "      Daily Costs: " + str(self.FormatCost(self._DailyCosts)) + "      Visits today: " + str(self._VisitsToday)
    return Details
...
class Company:
  ...
  def GetDetails(self):
    ...
    Details += "Current balance: " + str(self.FormatCost(self._Balance)) + "\nAverage cost per meal: " + str(self.FormatCost(self._AvgCostPerMeal)) + "\n"
    Details += "Average price per meal: " + str(self.FormatCost(self._AvgPricePerMeal)) + "\nDaily costs: " + str(self.FormatCost(self._DailyCosts)) + "\n"
    Details += "Delivery costs: " + str(self.FormatCost(self.CalculateDeliveryCost())) + "\nReputation: " + str(self._ReputationScore) + "\n\n"
  ...
  def ProcessDayEnd(self):
    ...
    Details += "Daily costs for company: " + str(self.FormatCost(self._DailyCosts)) + "\nCost for delivering produce to outlets: " + str(self.FormatCost(DeliveryCosts)) + "\n"
    for Current in range (0, len(self._Outlets)):
      ...
      Details += "Outlet " + str(Current + 1) + " profit/loss: " + str(self.FormatCost(ProfitLossFromThisOutlet)) + "\n"
      ProfitLossFromOutlets += ProfitLossFromThisOutlet
    Details += "Previous balance for company: " + str(self.FormatCost(self._Balance)) + "\n"
    self._Balance += ProfitLossFromOutlets - self._DailyCosts - DeliveryCosts
    Details += "New balance for company: " + str(self.FormatCost(self._Balance))
    return Details
...
class Simulation:
  def __init__(self):
    ...
    Choice = input("Enter L for a large settlement, enter S for a smallsettlement, anything else for a normal size settlement: ")
    if Choice == "L":
      ...
      FewerHouseholds = int(input("Enter an amount of households to remove from the settlement: "))
      while SmallX >= 1000 or SmallY >= 1000 or FewerHouseholds >= 250:
          if SmallX >= 1000:
              SmallX = int(input("Please enter a valid amount to subtract (under 1000)."))
          if SmallY >= 1000:
              SmallY = int(input("Please enter a valid amount to subtract (under 1000)."))
          if FewerHouseholds >= 250:
              FewerHouseholds = int(input("Please enter a valid amount to subtract (under 250)."))
      self._SimulationSettlement = SmallSettlement(SmallX, SmallY, FewerHouseholds)  
    else:
  ...
  def __ProcessCostOfFuelChangeEvent(self):
    ...
    if UpOrDown == 0:
      print("The cost of fuel has gone up by " + str(self.FormatCost(FuelCostChange)) + " for " + self._Companies[CompanyNo].GetName())
    else:
      print("The cost of fuel has gone down by " + str(self.FormatCost(FuelCostChange)) + " for " + self._Companies[CompanyNo].GetName())
      FuelCostChange *= -1
  ...
  def __ProcessCostChangeEvent(self):
    ...
    if CostToChange == 0:
      ...
      if UpOrDown == 0:
        print("The daily costs for " + self._Companies[CompanyNo].GetName() + " have gone up by " + str(self.FormatCost(AmountOfChange)))
      else:
        print("The daily costs for " + self._Companies[CompanyNo].GetName() + " have gone down by " + str(self.FormatCost(AmountOfChange)))
        AmountOfChange *= -1
      self._Companies[CompanyNo].AlterDailyCosts(AmountOfChange)
    else:
      AmountOfChange = random.randint(1, 9) / 10.0
      if UpOrDown == 0:
        print("The average cost of a meal for " + self._Companies[CompanyNo].GetName() + " has gone up by " + str(self.FormatCost(AmountOfChange)))
      else:
        print("The average cost of a meal for " + self._Companies[CompanyNo].GetName() + " has gone down by " + str(self.FormatCost(AmountOfChange)))
        AmountOfChange *= -1
  ...
  def ProcessDayEnd(self):
    ...
    for Counter in range (0, LoopMax + 1):
      ...
      if EatsOut:
        self._SimulationSettlement._Households[Counter].AteOutUpdate()
        ...
        self._SimulationSettlement._Households[Counter].NotEatOutUpdate()
  ...
  def KillCompany(self):
    CompanyName = input("Which company do you want to remove? ")
    Index = self.GetIndexOfCompany(CompanyName)
    if Index == -1:
        print("No company with that name exists.")
    else:
        Removed = self._Companies.pop(Index)
        print(f"{Removed.GetName()} has been hit by a meteor.")
